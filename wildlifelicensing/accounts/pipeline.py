    ...
    if requires_validation and send_validation:
        ...
        if "verification_code" in data:  # sign-in URL request to authenticate a session
            ...
            if not backend.strategy.validate_email(
                ...
                return backend.strategy.redirect(reverse("accounts:login_expired"))
            ...
            try:
                ...
                backend.strategy.session_set(
                    "email_validation_address", details["email"]
                )
            except Exception:
                return backend.strategy.redirect(reverse("accounts:login_retry"))
...
def logout_previous_session(backend, details, user=None, *args, **kwargs):
    ...
    if (
        request_data.get("verification_code")
        and details.get("email")
        and user
        ...
        if user.email != details["email"]:
            ...
            return strategy.redirect(
                "{}?verification_code={}&email={}".format(
                    strategy.request_path(),
