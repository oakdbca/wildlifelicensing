...
from wildlifelicensing.apps.returns.tests.helpers import (
    create_return,
    get_or_create_return_type,
)
from wildlifelicensing.apps.returns.utils import create_returns_due_dates

TEST_SPREADSHEET_PATH = os.path.join(
    "wildlifelicensing", "apps", "returns", "test_data", "regulation17.xlsx"
)
...
class ReturnsTestCase(TestCase):
    ...
    def test_lodge_nil_return(self):
        ...
        post_params = {"nil": True, "comments": "No survey taken"}
        response = self.client.post(
            reverse("wl_returns:enter_return", args=(self.ret.pk,)), post_params
        )
    ...
    def test_upload_return_spreadsheet(self):
        ...
        with open(TEST_SPREADSHEET_PATH, "rb") as fp:
            post_params = {"upload": True, "spreadsheet_file": fp}
            response = self.client.post(
                reverse("wl_returns:enter_return", args=(self.ret.pk,)), post_params
            )
    ...
    def test_lodge_return(self):
        ...
        for key, value in TEST_VALUES.items():
            post_params[f"regulation-17::{key}"] = value
        ...
        for key, value in (
            self.ret.returntable_set.first().returnrow_set.first().data.items()
        ):
...
class TestPermissions(TestCase):
    ...
    def test_add_log(self):
        ...
        payload = {"to": "user", "from": "test", "type": "email"}
        for user in allowed:
            self.client.login(user.email)
...
class TestLifeCycle(TestCase):
    ...
    def _issue_licence(self, licence_data, **kwargs):
        ...
        licence = app_helpers.issue_licence(
            application, kwargs.get("issuer", self.officer), licence_data=licence_data
        )
    ...
    def _lodge_reg17_return(self, ret, data=None):
        ...
        response = self.client.post(
            reverse("wl_returns:enter_return", args=(ret.pk,)), post_params
        )
        self.assertRedirects(
            response,
            reverse("home"),
    ...
    def test_curate_happy_path(self):
        ...
        self.assertEqual(
            sorted(["headers", "data", "name", "title"]), sorted(table.keys())
        )
        ...
        for expected_key, expected_value in expected_data.items():
            self.assertTrue(
                expected_key in sent_first_row, f"{expected_key} not in sent data"
            )
        ...
        payload = {"accept": True}
        for key, value in data.items():
            payload[f"regulation-17::{key}"] = value
        response = self.client.post(url, data=payload)
        self.assertRedirects(
            response,
            reverse("home"),
    ...
    def test_curate_request_amendments(self):
        ...
        self.assertEqual(
            resp_data["amendment_request"].get("reason"), payload["reason"]
        )
        ...
        expected_url = f"/returns/enter-return/{ret.pk}"
...
class TestUtils(TestCase):
    ...
    def test_create_returns_due_dates(self):
        ...
        monthly_frequency = 4
        due_dates = create_returns_due_dates(start_date, end_date, monthly_frequency)
        expected_due_dates = [date(2017, 10, 30), date(2018, 2, 28), date(2018, 6, 30)]
        self.assertEqual(due_dates, expected_due_dates)
