...
from wildlifelicensing.apps.main.models import WildlifeLicence, WildlifeLicenceType
...
def licence_issued_callback(sender, **kwargs):
    if "wildlife_licence" in kwargs:
        ...
        due_dates = create_returns_due_dates(
            licence.start_date, licence.end_date, licence.return_frequency
        )
        ...
        if existing_returns.count() > 0:
            ...
            if latest_edited_return is not None:
                due_dates = [
                    ...
                    if due_date > latest_edited_return.due_date
                ]

        returns = []
        for due_date in due_dates:
            returns.append(
                Return(licence=licence, return_type=return_type, due_date=due_date)
            )
