...
from wildlifelicensing.apps.returns.mixins import (
    UserCanCurateReturnMixin,
    UserCanEditReturnMixin,
    UserCanViewReturnMixin,
)
...
def _get_table_rows_from_post(table_name, post_data):
    ...
    by_column = {
        key.replace(table_namespace, ""): post_data.getlist(key)
        for key in post_data.keys()
        if key.startswith(table_namespace)
    }
...
class EnterReturnView(UserCanEditReturnMixin, TemplateView):
    ...
    def _set_submitted(self, ret):
        ...
        next_ret = (
            Return.objects.filter(licence=ret.licence, status="future")
            .order_by("due_date")
            .first()
        )
            ...
            if "draft" in request.POST:
                ...
                for table in context["tables"]:
                    table["data"] = _get_validated_rows_from_post(
                        ret, table.get("name"), request.POST
                    )
            ...
            if _is_post_data_valid(ret, context["tables"], request.POST):
                ...
                for table in context["tables"]:
                    ...
                    if len(table["data"]) == 0:
                        messages.warning(
                            request,
                            "You must enter data for {} or submit a Nil Return".format(
                                table.get("name")
                            ),
                        )
                    else:
                        messages.error(
                            request, "Your return contains some errors. See below."
                        )
            ...
            if _is_post_data_valid(ret, context["tables"], request.POST):
                ...
                for table in context["tables"]:
                    ...
                    if len(table["data"]) == 0:
                        messages.warning(
                            request,
                            "You must enter data for {}".format(table.get("name")),
                        )
...
class DownloadReturnTemplate(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        filename = f"Return_{return_type.licence_type.code}_template.xlsx"
