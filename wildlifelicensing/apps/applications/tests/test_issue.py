...
class TestIssueLicence(TestCase):
    ...
    def setUp(self):
        ...
        self.lodged_application = app_helpers.create_and_lodge_application(
            self.user, **{"data": {"title": "Lodged Application"}}
        )
        self.issued_application = app_helpers.create_and_lodge_application(
            self.user, **{"data": {"title": "Issued Application"}}
        )
        ...
        self.issue_urls_get = [
            {
                "url": reverse(
                    "wl_applications:issue_licence", args=[self.lodged_application.pk]
                ),
                "data": None,
            },
            {
                "url": reverse(
                    "wl_applications:reissue_licence", args=[self.licence.pk]
                ),
                "data": None,
            },
            {
                "url": reverse(
                    "wl_applications:preview_licence", args=[self.issued_application.pk]
                ),
    ...
    def test_save_licence_details_from_issue_page(self):
        ...
        self.assertNotEqual(application.processing_status, "issued")
