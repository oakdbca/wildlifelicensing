...
class TestViewAccess(TestCase):
    ...
    def setUp(self):
        ...
        self.urls_post = [
            {
                "url": reverse(
                    "wl_applications:create_condition", args=[self.application.pk]
                ),
                "data": {"code": "123488374", "text": "condition text"},
            },
            {
            ...
            {
                "url": reverse(
                    "wl_applications:enter_conditions", args=[self.application.pk]
                ),
                "data": {
    ...
    def test_assessor_access_normal(self):
        ...
        self.assertTrue(
            Assessment.objects.filter(application=self.application).count() > 0
        )
...
class TestAssignAssessor(TestCase):
    ...
    def setUp(self):
        ...
        self.assessor_group = G(
            AssessorGroup, name="District7", email="district7@test.com"
        )
    ...
    def _issue_assessment(self, application, assessor_group):
        ...
        payload = {"applicationID": application.pk, "assGroupID": assessor_group.pk}
    ...
    def test_assign_assessment_send_email(self):
        ...
        payload = {"assessmentID": assessment.id, "userID": self.assessor_2.id}
        resp = self.client.post(url, data=payload)
        self.assertEqual(resp.status_code, 200)
        # the response is a json response. It should contain the assessment id
        expected_content_type = "application/json"
    ...
    def test_assign_to_me_no_email(self):
        ...
        url = reverse("wl_applications:assign_assessor")
        self.client.login(self.assessor_1.email)
        payload = {"assessmentID": assessment.id, "userID": self.assessor_1.id}
        resp = self.client.post(url, data=payload)
        # the response is a json response. It should contain the assessment id
        expected_content_type = "application/json"
        self.assertEqual(resp["content-type"], expected_content_type)
