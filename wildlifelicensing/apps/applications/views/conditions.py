...
class EnterConditionsView(OfficerRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            application.processing_status not in ["issued", "declined"]
            and request.POST.get("submissionType") != "save"
        ):
        ...
        application.log_user_action(
            ApplicationUserAction.ACTION_ENTER_CONDITIONS, request
        )
...
class EnterConditionsAssessorView(CanPerformAssessmentMixin, TemplateView):
    ...
    def _check_read_only(self, assessment, request):
        if assessment.status == "assessed":
            messages.warning(
                request,
                """This assessment has already been concluded and may only be viewed in
            read-only mode.""",
            )
            return True
            ...
            messages.warning(
                request,
                """The assessment period for this application has expired, likely due to the
...
class SetAssessmentConditionState(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        assessment_condition = get_object_or_404(
            AssessmentCondition, pk=request.POST.get("assessmentConditionID")
        )

        assessment_condition.acceptance_status = request.POST.get("acceptanceStatus")
        assessment_condition.save()

        response = ASSESSMENT_CONDITION_ACCEPTANCE_STATUSES[
            assessment_condition.acceptance_status
        ]
...
class AssignAssessorView(OfficerOrAssessorRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            assessment.assigned_assessor = EmailUser.objects.get(
        ...
        if assessment.assigned_assessor is not None:
            ...
            assessment.application.log_user_action(
                ApplicationUserAction.ACTION_ASSESSMENT_ASSIGN_TO_.format(name), request
            )
            ...
            assessment.application.log_user_action(
                ApplicationUserAction.ACTION_ASSESSMENT_UNASSIGN, request
            )
