...
class IssueLicenceView(OfficerRequiredMixin, TemplateView):
    ...
    def _issue_licence(self, request, application, issue_licence_form):
        ...
        # for re-issuing
        original_issue_date = (
            application.licence.issue_date if application.licence.is_issued else None
        )
        ...
        licence_filename = "licence-%s-%d.pdf" % (
        ...
        ccs = None
        if (
            "ccs" in issue_licence_form.cleaned_data
            and issue_licence_form.cleaned_data["ccs"]
        ):
    ...
    def get_context_data(self, **kwargs):
        ...
        if application.licence:
            ...
            if (
                application.previous_application is not None
                and application.previous_application.licence is not None
            ):
                previous_licence = application.previous_application.licence

                licence_initial_data["regions"] = (
                    previous_licence.regions.all().values_list("pk", flat=True)
                )
                licence_initial_data["locations"] = previous_licence.locations
                licence_initial_data["purpose"] = previous_licence.purpose
                licence_initial_data["additional_information"] = (
                    previous_licence.additional_information
                )
                licence_initial_data["cover_letter_message"] = (
                    previous_licence.cover_letter_message
                )
            ...
            if (
                "purpose" not in licence_initial_data
                or len(licence_initial_data["purpose"]) == 0
            ):
            ...
            if hasattr(application.licence_type, "returntype"):
                licence_initial_data["return_frequency"] = (
                    application.licence_type.returntype.month_frequency
                )
            else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if application.licence is not None:
            issue_licence_form = IssueLicenceForm(
                request.POST,
                instance=application.licence,
                files=request.FILES,
            ...
            issue_licence_form = IssueLicenceForm(
                request.POST, files=request.FILES, skip_required=skip_required
            )
        ...
        if issue_licence_form.is_valid():
            ...
            for index, avl in enumerate(
                ...
                WildlifeLicenceVariantLink.objects.create(
                    licence=licence, variant=avl.variant, order=index
                )
...
class PreviewLicenceView(OfficerRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if application.licence is not None:
            issue_licence_form = IssueLicenceForm(
                request.GET, instance=application.licence, skip_required=True
            )
        ...
        if issue_licence_form.is_valid():
            ...
            licence.profile = application.applicant_profile
            licence.holder = application.applicant
            licence.extracted_fields = update_licence_fields(
                extracted_fields, request.GET
            )
