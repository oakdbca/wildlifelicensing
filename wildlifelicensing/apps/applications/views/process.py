...
from wildlifelicensing.apps.main.models import AssessorGroup, Document
...
class ProcessView(OfficerOrAssessorRequiredMixin, TemplateView):
    ...
    def _build_data(self, request, application):
        officers = [
            {"id": officer.id, "text": render_user_name(officer)}
            for officer in get_all_officers()
        ]
        officers.insert(0, {"id": 0, "text": "Unassigned"})

        current_ass_groups = [
            ass_request.assessor_group
            for ass_request in Assessment.objects.filter(application=application)
        ]

        ass_groups = [
            {"id": ass_group.id, "text": ass_group.name}
            for ass_group in AssessorGroup.objects.all()
            .exclude(id__in=[ass_group.pk for ass_group in current_ass_groups])
            .order_by("name")
        ]
        ...
        previous_lodgements = []
        for revision in (
            Version.objects.get_for_object(application)
            .filter(revision__comment="Details Modified")
            ...
            if previous_lodgement.hard_copy is not None:
                (
                    previous_lodgement.licence_type.application_schema,
            ...
            convert_documents_to_url(
                previous_lodgement.data, previous_lodgement_documents, ""
            )
            previous_lodgements.append(
                {
                    "lodgement_number": "{}-{}".format(
                        previous_lodgement.lodgement_number,
                        previous_lodgement.lodgement_sequence,
                    ),
                    "date": formats.date_format(
                        revision.revision.date_created, "d/m/Y", True
                    ),
        ...
        if application.previous_application is not None:
            previous_application_returns_outstanding = (
                Return.objects.filter(licence=application.previous_application.licence)
    ...
    def _process_decline(self, application):
        ...
        if form.is_valid():
            ...
            Assessment.objects.filter(
                application=application, status="awaiting_assessment"
            ...
            return True, f"Application declined and email sent to {recipient}"
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["id_request_form"] = IDRequestForm(
            application=application, officer=self.request.user
        )
        kwargs["returns_request_form"] = ReturnsRequestForm(
            application=application, officer=self.request.user
        )
        kwargs["amendment_request_form"] = AmendmentRequestForm(
            application=application, officer=self.request.user
        )
        kwargs["log_entry_form"] = ApplicationLogEntryForm(
            to=get_log_entry_to(application), fromm=self.request.user.get_full_name()
        )
        kwargs["application_declined_details_form"] = ApplicationDeclinedDetailsForm(
            application=application, officer=self.request.user
        )
...
class AssignOfficerView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        if application.assigned_officer is not None:
            ...
            application.log_user_action(
                ApplicationUserAction.ACTION_ASSIGN_TO_.format(name), request
            )
        else:
            assigned_officer = {"id": 0, "text": "Unassigned"}
            application.log_user_action(ApplicationUserAction.ACTION_UNASSIGN, request)
...
class IDRequestView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        if id_request_form.is_valid():
            ...
            application.save()
            application.log_user_action(
                ApplicationUserAction.ACTION_ID_REQUEST_UPDATE, request
            )
...
class ReturnsRequestView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        if returns_request_form.is_valid():
            ...
            response = {
                "returns_check_status": RETURNS_CHECK_STATUSES[
                    application.returns_check_status
                ],
                "processing_status": PROCESSING_STATUSES[application.processing_status],
            }
...
class SetReturnsCheckStatusView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {
                "returns_check_status": CHARACTER_CHECK_STATUSES[
                    application.returns_check_status
                ],
                "processing_status": PROCESSING_STATUSES[application.processing_status],
            },
...
class SetCharacterCheckStatusView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {
                "character_check_status": CHARACTER_CHECK_STATUSES[
                    application.character_check_status
                ],
                "processing_status": PROCESSING_STATUSES[application.processing_status],
            },
            safe=False,
...
class SetReviewStatusView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        response = {
            "review_status": REVIEW_STATUSES[application.review_status],
            "processing_status": PROCESSING_STATUSES[application.processing_status],
        }
...
class AmendmentRequestView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        amendment_request_obj = {
            k: amendment_request_attrs[k]
            for k in amendment_request_attrs
            if not k.startswith("_")
        }
        amendment_request_obj["creator"] = {
            "id": amendment_request.creator.id,
            "text": render_user_name(amendment_request.creator),
        }
        ...
        response = {
            ...
            "amendment_request": serialize(
                amendment_request_obj, posthook=format_amendment_request
            ),
        }
...
class SendForAssessmentView(OfficerRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        assessment = Assessment.objects.get_or_create(
            ...
            defaults={"officer": self.request.user},
        )[0]
...
def determine_processing_status(application):
    ...
    if (
        application.id_check_status == "awaiting_update"
        or application.review_status == "awaiting_amendments"
    ):
        status = "awaiting_applicant_response"

    if (
        Assessment.objects.filter(application=application)
        .filter(status="awaiting_assessment")
        .exists()
    ):
