...
from wildlifelicensing.apps.applications.mixins import (
    RedirectApplicationInSessionMixin,
    RedirectApplicationNotInSessionMixin,
    UserCanAmendApplicationMixin,
    UserCanEditApplicationMixin,
    UserCanRenewApplicationMixin,
...
class DeleteApplicationSessionView(UserCanEditApplicationMixin, View):
    def post(self, request, *args, **kwargs):
        application = get_object_or_404(
            Application, pk=request.POST.get("applicationId")
        )
...
class NewApplicationView(
    OfficerOrCustomerRequiredMixin, RedirectApplicationInSessionMixin, View
):
...
class EditApplicationView(
    UserCanEditApplicationMixin, RedirectApplicationInSessionMixin, View
):
    def get(self, request, *args, **kwargs):
        utils.remove_temp_applications_for_user(request.user)

        try:
            utils.set_session_application(
                request.session, Application.objects.get(id=args[0])
            )
...
class DiscardApplicationView(View, UserCanViewApplicationMixin):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            ctx = {
                "action_url": reverse(
                    "wl_applications:discard_application", args=[application.pk]
                ),
                "cancel_url": reverse("wl_dashboard:home"),
            }
            return render(
                request, template_name="wl/entry/confirm_discard.html", context=ctx
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if application.is_deletable:
                ...
                application.customer_status = application.processing_status = (
                    "discarded"
                )
                application.save()
                application.log_user_action(
                    ApplicationUserAction.ACTION_DISCARD_APPLICATION.format(
                        application
                    ),
                    request,
                )
            if application.lodgement_number:
                message = f"Application {application} discarded"
            else:
...
class RenewLicenceView(
    UserCanRenewApplicationMixin, RedirectApplicationInSessionMixin, View
):
    def get(self, request, *args, **kwargs):
        utils.remove_temp_applications_for_user(request.user)
...
class AmendLicenceView(
    UserCanAmendApplicationMixin, RedirectApplicationInSessionMixin, View
):
    def get(self, request, *args, **kwargs):
        utils.remove_temp_applications_for_user(request.user)
...
class CreateSelectCustomer(
    OfficerRequiredMixin, RedirectApplicationNotInSessionMixin, TemplateView
):
...
class SelectLicenceTypeView(
    LoginRequiredMixin, RedirectApplicationNotInSessionMixin, TemplateView
):
    ...
    def get(self, request, *args, **kwargs):
            ...
            for licence_type in licence_type_queryset:
                ...
                if licence_type.variant_group is not None:
                    ...
                    licence_type_dict["href"] = reverse(
                        "wl_applications:select_licence_type", args=(licence_type.id,)
                    )
        ...
        for category in WildlifeLicenceCategory.objects.all():
            __populate_category_dict(
                {"name": category.name, "licence_types": []},
                WildlifeLicenceType.objects.filter(
                    category=category, replaced_by__isnull=True
                ).exclude(effective_to__lte=datetime.now()),
                categories,
            )
        ...
        if uncategorised_queryset.exists():
            __populate_category_dict(
                {"name": "Other", "licence_types": []},
                uncategorised_queryset,
                categories,
            )
        ...
        return render(
            request,
            self.template_name,
            {"application": application, "categories": categories},
        )


class CheckIdentificationRequiredView(
    LoginRequiredMixin, ApplicationEntryBaseView, FormView
):
...
class EnterDetailsView(UserCanEditApplicationMixin, ApplicationEntryBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if application.data:
            utils.convert_documents_to_url(
        ...
        for f in request.FILES:
            if f == "application_document":
                if application.hard_copy is None:
                    application.hard_copy = LocalDocument.objects.create(
                        name="hard_copy"
                    )
                application.hard_copy.file = request.FILES[f]
                application.hard_copy.save()
            else:
                document = LocalDocument.objects.create(name=f, file=request.FILES[f])
                document.save()
...
class PreviewView(UserCanEditApplicationMixin, ApplicationEntryBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["is_payment_required"] = (
            not is_licence_free(generate_product_title(application))
        ...
        if application.hard_copy is not None:
            application.licence_type.application_schema, application.data = (
                utils.append_app_document_to_schema_data(
    ...
    def post(self, request, *args, **kwargs):
        ...
        application.correctness_disclaimer = (
            request.POST.get("correctnessDisclaimer", "") == "on"
        )
        application.further_information_disclaimer = (
            request.POST.get("furtherInfoDisclaimer", "") == "on"
        )
...
class ApplicationCompleteView(UserCanViewApplicationMixin, ApplicationEntryBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        application.log_user_action(
            ApplicationUserAction.ACTION_LODGE_APPLICATION.format(application), request
        )
        ...
        context["show_invoice"] = (
            not is_licence_free(generate_product_title(application))
            and not application.application_type == "amendment"
        )
