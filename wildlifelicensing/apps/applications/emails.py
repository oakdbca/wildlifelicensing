...
def _format_application_email_subject(subject, application):
    subject = subject.format(
        application.reference, application.applicant.get_full_name()
    )

    # subject can be no longer than MAX_SUBJECT_LENGTH
    if len(subject) > MAX_SUBJECT_LENGTH:
        subject = f"{subject[:MAX_SUBJECT_LENGTH - 2]}.."
...
def send_amendment_requested_email(amendment_request, request):
    ...
    if amendment_request.reason:
        context["reason"] = dict(AmendmentRequest.REASON_CHOICES)[
            amendment_request.reason
        ]
...
def send_assessment_done_email(assessment, request):
    ...
    url = request.build_absolute_uri(
        reverse("wl_applications:enter_conditions", args=[application.pk])
    )
    context = {
...
def send_id_update_request_email(id_request, request):
    ...
    context = {"id_request": id_request, "url": url}
...
def send_returns_request_email(returns_request, request):
    ...
    if returns_request.reason:
        returns_request.reason = dict(ReturnsRequest.REASON_CHOICES)[
            returns_request.reason
        ]

    context = {"url": url, "returns_request": returns_request}
...
def send_licence_issued_email(
    ...
    if licence.licence_document is not None:
        ...
        attachment = (
            file_name,
            licence.licence_document.file.read(),
            "application/pdf",
        )
        attachments = [attachment]
    else:
        logger.error(
            "The licence pk=" + licence.pk + " has no document associated with it."
        )
        attachments = []
    if not to:
