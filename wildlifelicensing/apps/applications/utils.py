...
ASSESSMENT_CONDITION_ACCEPTANCE_STATUSES = dict(
    AssessmentCondition.ACCEPTANCE_STATUS_CHOICES
)
    ...
    if item.get("isLicenceField", False):
        if "children" not in item:
            ...
            for index in range(len(child_data)):
                ...
                for child_item in item.get("children"):
                    if child_item["type"] == "label" and child_item.get(
                        "isLicenceField", False
                    ):
                        _extract_label_and_checkboxes(
                            child_item,
                            item.get("children"),
                            child_data[index],
                            group_licence_fields,
                    ...
                    _extract_licence_fields_from_item(
                        child_item, child_data[index], group_licence_fields
                    )
    ...
    if "children" in item and not children_extracted:
        for child_item in item.get("children"):
            if child_item["type"] == "label" and child_item.get(
                ...
                _extract_label_and_checkboxes(
                    child_item, item.get("children"), data, licence_fields
                )
        ...
        for condition in item["conditions"].keys():
            for child_item in item["conditions"][condition]:
                if child_item["type"] == "label" and child_item.get(
                    ...
                    _extract_label_and_checkboxes(
                        child_item, item["conditions"][condition], data, licence_fields
                    )
                _extract_licence_fields_from_item(child_item, data, licence_fields)
...
def _extract_label_and_checkboxes(current_item, items, data, licence_fields):
    ...
    checkbox_index = 0
    while (
        checkbox_index < len(items)
        and items[checkbox_index]["name"] != current_item["name"]
    ):
        checkbox_index += 1
    checkbox_index += 1
    ...
    while checkbox_index < len(items) and items[checkbox_index]["type"] == "checkbox":
        ...
        option = {"name": name, "label": items[checkbox_index]["label"]}
        if name in data:
...
def _create_licence_field(item):
    return {
        ...
        "type": item["type"],
        "label": (
            item["licenceFieldLabel"] if "licenceFieldLabel" in item else item["label"]
        ),
            ...
            if isinstance(value, list):
                for rep in range(0, len(value)):
                    convert_documents_to_url(
                        value[rep], document_queryset, f"{suffix}-{rep}"
                    )
            else:
                try:
                    ...
                    print("value: ", value)
                except LocalDocument.DoesNotExist:
                    try:
                        data[item] = document_queryset.get(
                            name=f"{item}{suffix}-0"
                        ).file.url
                    except LocalDocument.DoesNotExist:
                        pass
...
def determine_applicant(request):
    if "application" in request.session:
        if "customer_pk" in request.session.get("application"):
            try:
                applicant = EmailUser.objects.get(
                    pk=request.session["application"]["customer_pk"]
                )
            except EmailUser.DoesNotExist:
                raise SessionDataMissingException(
                    "customer_pk does not refer to existing customer"
                )
            except EmailUser.MultipleObjectsReturned:
                raise SessionDataMissingException(
                    "customer_pk does not refer to several customers"
                )
        else:
...
def get_session_application(session):
    ...
    try:
        ...
        raise Exception(f"Application not found for application_id {application_id}")
...
def remove_temp_applications_for_user(user):
    if is_customer(user):
        ...
        Application.objects.filter(
            proxy_applicant=user, customer_status="temp"
        ).delete()
...
def clone_application_with_status_reset(application, is_licence_amendment=False):
    ...
    application.previous_application = Application.objects.get(
        pk=original_application_pk
    )
    ...
    # clone variants
    for application_variant in Application.variants.through.objects.filter(
        application=original_application_pk
    ):
    ...
    # clone documents
    for application_document in Application.documents.through.objects.filter(
        application=original_application_pk
    ):
    ...
    # clone conditions
    for application_condition in ApplicationCondition.objects.filter(
        application=original_application_pk
    ):
...
def append_app_document_to_schema_data(schema, data, app_doc):
    section = {
        "type": "section",
        "label": "Original Application LocalDocument",
        "name": "original_application_document",
    }
    section["children"] = [
        {
            "type": "file",
            "label": "Application LocalDocument File",
...
def format_application(instance, attrs):
    ...
    attrs["conditions"] = serialize(
        [ap.condition for ap in instance.applicationcondition_set.order_by("order")]
    )
    ...
    attrs["applicant_profile"]["postal_address"] = serialize(
        instance.applicant_profile.postal_address, exclude=["user", "oscar_address"]
    )
    ...
    if (
        instance.applicant.identification2 is not None
        and instance.applicant.identification2.upload is not None
    ):
        attrs["applicant"]["identification2"][
            "url"
        ] = instance.applicant.identification2.upload.url
    if (
        instance.applicant.senior_card2 is not None
        and instance.applicant.senior_card2.upload is not None
    ):
        attrs["applicant"]["senior_card2"][
            "url"
        ] = instance.applicant.senior_card2.upload.url
...
def format_assessment_condition(instance, attrs):
    attrs["acceptance_status"] = ASSESSMENT_CONDITION_ACCEPTANCE_STATUSES[
        attrs["acceptance_status"]
    ]
