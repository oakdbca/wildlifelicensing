define([
  ...
  function _layoutItem(item, isRepeat, itemDataCurrent, itemDataPrevious) {
    ...
    if (item.type === "section" || item.type === "group") {
      ...
      if (
        (item.valueCurrent === item.valuePrevious && item.valueCurrent) ||
        (item.valuePrevious === undefined && isRepeat)
      ) {
        ...
        if (item.valueCurrent) {
          itemContainer.append(
            $("<p>").text(item.label).addClass("current-data")
          );
        } else if (item.valuePrevious) {
          itemContainer.append(
            $("<p>").text(item.label).addClass("previous-data")
          );
        }
      }
      ...
      if (
        ...
        if (item.valueCurrent) {
          ...
          currentFileLink.text(
            item.valueCurrent.substr(item.valueCurrent.lastIndexOf("/") + 1)
          );
        ...
        if (item.valueCurrent && item.valuePrevious) {
          ...
          previousFileLink.text(
            item.valuePrevious.substr(item.valuePrevious.lastIndexOf("/") + 1)
          );
          previousFileLink.addClass("previous-data");

          itemContainer.append($("<p>").append(currentFileLink));
          itemContainer.append(
            $("<p>").append(previousFileLink).addClass("previous-data")
          );
          ...
          itemContainer.append($("<p>").append(currentFileLink));
          itemContainer.append(
            $("<p>").addClass("previous-data").text("Not specified")
          );
        } else if (!item.valueCurrent && item.valuePrevious) {
          previousFileLink.attr("href", item.valuePrevious);
          previousFileLink.attr("target", "_blank");
          ...
          itemContainer.append(
            $("<p>").addClass("current-data").text("Not specified")
          );
      ...
      if (
        ...
        itemContainer.append(
          $("<p>").addClass("current-data").text(item.valueCurrent)
        );
        itemContainer.append(
          $("<p>").addClass("previous-data").text(item.valuePrevious)
        );
      }
    }
  ...
  function _appendChild(
    ...
    if (child.isRepeatable) {
      ...
      childrenAnchorPoint.append(
        _layoutItem(child, false, childDataCurrent, childDataPrevious)
      );

      childrenAnchorPoint.append(repeatItemsAnchorPoint);

      if (
        (itemDataCurrent !== undefined &&
          child.name in itemDataCurrent &&
          itemDataCurrent[child.name].length > 1) ||
        (itemDataPrevious !== undefined &&
          child.name in itemDataPrevious &&
          itemDataPrevious[child.name].length > 1)
      ) {
        var itemDataLength;
        if (
          itemDataCurrent[child.name] !== undefined &&
          itemDataPrevious[child.name].length
        ) {
          itemDataLength = Math.max(
            itemDataCurrent[child.name].length,
            itemDataPrevious[child.name].length
          );
        ...
        for (var i = 1; i < itemDataLength; i++) {
          repeatItemsAnchorPoint.append(
            _layoutItem(
              child,
              true,
              itemDataCurrent[child.name][i],
              itemDataPrevious[child.name][i]
            )
          );
        }
      }
      ...
      childrenAnchorPoint.append(
        _layoutItem(child, false, itemDataCurrent, itemDataPrevious)
      );
    }
  }
  ...
  return {
    layoutPreviewItems: function (
      ...
      for (var i = 0; i < formStructure.length; i++) {
        var currentItemData, previousItemData;
