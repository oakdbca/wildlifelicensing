  ...
  function _layoutItem(item, repetition, suffix, itemData) {
    ...
    if (item.conditions !== undefined) {
      ...
      if (initialInputValue in item.conditions) {
        ...
        $.each(
          item.conditions[initialInputValue],
      ...
      $input.change(function (e) {
        ...
        $slideUpPromise = $internalChildrenAnchorPoint
          .children()
          .slideUp("medium")
          .promise();
        ...
        if (inputValue in item.conditions) {
          ...
          $conditionalChildren = $internalChildrenAnchorPoint.find(
            "#" + inputValue
          );
  ...
  function _appendChild(
    ...
    if (child.isRepeatable) {
      ...
      $childrenAnchorPoint.append(
        _layoutItem(child, repetition, suffix, childData)
      );
        ...
        $.each(
          ...
          function (childRepetitionIndex, repeatData) {
            repeatItemsAnchorPoint.append(
              _layoutItem(child, childRepetitionIndex + 1, suffix, repeatData)
            );
      ...
      $childrenAnchorPoint.append(
        _layoutItem(child, repetition, suffix, itemData)
      );
    }
  }
    ...
    if (item.type === "species") {
      ...
      $itemContainer.find(".species").typeahead({
        ...
        source: function (query, process) {
          return $.get(
            "/taxonomy/species_name?search=" + query + species_type_arg,
            function (data) {
              return process(data);
            }
          );
