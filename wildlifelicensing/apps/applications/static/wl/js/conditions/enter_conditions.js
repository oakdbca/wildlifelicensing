define([
    ...
    $.each(assessments, function (assessmentIndex, assessment) {
      if (assessment.status === "Assessed") {
        ...
        $contentContainer.append(
          $("<label>").text("Assessor's Suggested Conditions")
        );

        if (assessment.conditions.length > 0) {
          var $assessorsConditionsTable = $("<table>")
              .addClass("table")
              .addClass("table-bordered")
              .addClass("popover-conditions-table"),
            $assessorsConditionsTableHead = $("<thead>").addClass(
              "popover-conditions-table-head"
            ),
            $assessorsConditionsTableBody = $("<tbody>").addClass(
              "conditions-table-body"
            );

          $assessorsConditionsTable
            .append($assessorsConditionsTableHead)
            .append($assessorsConditionsTableBody);

          $assessorsConditionsTableHead.append(
            $("<tr>")
              .append($("<th>").addClass("code").text("Code"))
              .append($("<th>").addClass("condition").text("Condition"))
              .append(
                $("<th>").addClass("action-status").text("Action / Status")
              )
          );

          $.each(
            assessment.conditions,
            function (conditionIndex, assessmentCondition) {
              var $conditionRow = $("<tr>").addClass("assessor"),
                $actionCell = $("<td>");

              $conditionRow.append(
                $("<td>").text(assessmentCondition.condition.code)
              );
              $conditionRow.append(
                $("<td>").text(assessmentCondition.condition.text)
              );
              ...
              if (assessmentCondition.acceptance_status === "Not Specified") {
                ...
                $accept.click(function (e) {
                  var existingConditions =
                    $conditionsForm.find("input[type=hidden]");
                  ...
                  if (
                    !_.includes(
                      ...
                      String(assessmentCondition.condition.id),
                      1
                    )
                  ) {
                    $.post(
                      "/applications/set-assessment-condition-state/",
                      {
                      ...
                      function (acceptanceStatus) {
                        assessmentCondition.acceptance_status =
                          acceptanceStatus;
                        createConditionTableRow(
                          assessmentCondition.condition,
                          "assessor"
                        );
                        $actionCell.html(
                    ...
                    window.alert(
                      "The specified condition has already been entered."
                    );
                  }
                });
                ...
                $actionCell.html(
                  $("<div>").append($accept).append($("<hr>")).append($decline)
                );
              } else {
                $actionCell.html(
                  $("<p>").text(assessmentCondition.acceptance_status)
          ...
          $contentContainer.append(
            $("<p>").append($("<em>").text("No conditions specified"))
          );
        }
        ...
        if (typeof bootstrap !== "undefined" && bootstrap.Popover) {
          new bootstrap.Popover($viewFeedback[0], {
            container: "body",
            title:
              "Conditions / Comments from " + assessment.assessor_group.name,
            content: $contentContainer,
            html: true,
          });
        } else {
  ...
  function initExistingConditions(application, assessments) {
    ...
    $.each(application.conditions, function (index, condition) {
      if (
        _.some(application.licence_type.default_conditions, [
          "id",
          condition.id,
        ])
      ) {
        ...
        _.some(
          _.map(assessorConditions, function (assessorCondition) {
    ...
    $("#createCustomCondition").click(function (e) {
      ...
      if (typeof bootstrap !== "undefined" && bootstrap.Modal) {
        bootstrap.Modal.getOrCreateInstance(
          $createCustomConditionModal.get(0)
        ).show();
      } else if ($createCustomConditionModal.modal) {
