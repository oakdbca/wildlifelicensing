...
styles.add(
    ParagraphStyle(
        ...
        spaceAfter=HEADER_MARGIN,
        alignment=enums.TA_CENTER,
    )
)
styles.add(
    ParagraphStyle(
        name="ApplicationVariantsTitle",
        fontName=BOLD_FONTNAME,
...
def _create_application(application_buffer, application):
    ...
    doc = BaseDocTemplate(
        application_buffer, pageTemplates=[every_page_template], pagesize=A4
    )

    elements = []

    elements.append(
        Paragraph(application.licence_type.name, styles["ApplicationTitle"])
    )
...
def _create_application_metadata(application):
    ...
    lodgement_date = (
        application.lodgement_date.strftime(settings.DEFAULT_FORM_DATE_FORMAT)
        if application.lodgement_date is not None
        else ""
    )
    ...
    return Table(
        [top_row, bottow_row],
        colWidths=[
            left_heading_cell_width,
            value_cell_width,
            right_heading_cell_width,
            value_cell_width,
        ],
...
def _create_application_questionaire(field, data, elements, indent_index):
    paragraph_style = ParagraphStyle(
        ...
        leftIndent=indent_index * PARAGRAPH_INDENT,
    )
    if field["type"] == "section":
        ...
        for child_data in data[field["name"]]:
            for child_field in field["children"]:
                _create_application_questionaire(
                    child_field, child_data, elements, indent_index
                )
        ...
        for group_index, child_data in enumerate(data.get(field["name"], [])):
            group_label = (
                "{} {}".format(field["label"], group_index + 1)
            ...
            for child_field in field["children"]:
                _create_application_questionaire(
                    child_field, child_data, elements, indent_index + 1
                )
    else:
        paragraph_style.fontName = BOLD_FONTNAME
        ...
        if "conditions" in field and data.get(field["name"]) in field["conditions"]:
            for child_field in field["conditions"][data.get(field["name"])]:
                _create_application_questionaire(
                    child_field, data, elements, indent_index + 1
                )
