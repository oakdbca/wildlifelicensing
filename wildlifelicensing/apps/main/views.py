...
class EditAccountView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            ...
            self.template_name,
            {
                "emailuser_form": EmailUserForm(instance=emailuser),
            },
        )
        ...
        if "discontinue" in request.POST:
            ...
            if emailuser_form.is_valid():
                ...
                if new_user and not unique:
                    message = """This combination of given name(s),
                    last name and date of birth is not unique.
                    If you already have a Parks and Wildlife customer
                    account under another email address, please discontinue
                    this registration and sign in with your existing account
                    and add any additional email addresses as new profiles."""
        ...
        if form.is_valid():
            ...
            if request.FILES and "attachment" in request.FILES:
                communications_log_entry.documents.add(
                    LocalDocument.objects.create(file=request.FILES["attachment"])
                )
...
def getLedgerIdentificationFile(request, emailuser_id):
    ...
    try:
        ...
        messages.error(request, "Unable to find the email user with id " + emailuser_id)
