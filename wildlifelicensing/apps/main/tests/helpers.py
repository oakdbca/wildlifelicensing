...
def get_or_create_default_customer(include_default_profile=False):
    ...
    if include_default_profile:
        ...
        profile = Profile.objects.create(
            user=user, postal_address=address, **TestData.DEFAULT_PROFILE
        )
        profile.user = user
...
def get_or_create_default_assessor_group():
    return AssessorGroup.objects.get_or_create(
        defaults=TestData.DEFAULT_ASSESSOR_GROUP,
        name=TestData.DEFAULT_ASSESSOR_GROUP["name"],
    )[0]
...
def get_response_error_messages(response):
    return [
        m for m in get_response_messages(response) if m.level == message_constants.ERROR
    ]
...
class BaseUserTestCase(TestCase):
    ...
    def _pre_setup(self):
        ...
        self.all_users = [self.officer, self.assessor, self.customer]
    ...
    def test_permissions(self):
        ...
        for verb in verbs:
            ...
            if callable(method):
                if verb in permissions:
                    ...
                    for user in allowed:
                        ...
                        self.assertEqual(
                            ...
                            expected_status,
                            msg=(
                                "{got} != {expected} for user {user} with method "
                                "{verb} at {url} with params {kwargs}"
                            ).format(
                    ...
                    for user in forbidden:
                        ...
                        self.assertEqual(
                            ...
                            expected_status,
                            msg=(
                                "{got} != {expected} for user {user} with method {verb} "
                                "at {url} with params {kwargs}"
                            ).format(
                                got=response.status_code,
                                expected=expected_status,
                                user=user.email,
                ...
                self.fail(f"Method {verb} is not supported by client.")
