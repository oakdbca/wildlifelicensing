...
def _create_licence_header(canvas, doc, draw_page_number=True):
    ...
    canvas.drawCentredString(
        PAGE_WIDTH / 2, current_y - LARGE_FONTSIZE, "DEPARTMENT OF PARKS AND WILDLIFE"
    )
    ...
    canvas.drawString(
        current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER), "Enquiries:"
    )
    canvas.drawString(
        current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 2, "Telephone:"
    )
    canvas.drawString(
        current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 3, "Facsimile:"
    )
    canvas.drawString(
        current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 4, "Web Site:"
    )
    canvas.drawString(
        current_x,
    ...
    canvas.drawString(
        current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 4, doc.site_url
    )
    ...
    if (
        hasattr(doc, "licence")
        and doc.licence.licence_number is not None
        and doc.licence.licence_sequence
    ):
        canvas.drawString(
...
def _create_licence(
    licence_buffer, licence, application, site_url, original_issue_date
):
    ...
    every_page_template = PageTemplate(
        id="EveryPages", frames=[every_page_frame], onPage=_create_licence_header
    )

    doc = BaseDocTemplate(
        licence_buffer, pageTemplates=[every_page_template], pagesize=A4
    )
    ...
    elements.append(
        Paragraph(licence.licence_type.code.upper(), styles["InfoTitleLargeCenter"])
    )
    ...
    if application.variants.exists():
        title = "{} ({})".format(
            ...
            " / ".join(application.variants.all().values_list("name", flat=True)),
        )
    else:
    ...
    elements.append(
        Paragraph(licence.licence_type.statement, styles["InfoTitleLargeLeft"])
    )
    elements.append(
        Paragraph(licence.licence_type.authority, styles["InfoTitleLargeRight"])
    )
    ...
    address_paragraphs = [
        ...
        Paragraph(
            f"{address.locality} {address.state} {address.postcode}",
            styles["Left"],
        ),
            ...
            [
                [
                    ...
                    [
                        Paragraph(
                            _format_name(
                                application.applicant, include_first_name=True
                            ),
                            styles["Left"],
                        )
                    ]
    ...
    for field in extracted_fields:
        if "children" not in field:
            if "data" in field and field["data"]:
                ...
                if any(
                    [option.get("data", "off") == "on" for option in field["options"]]
                ):
                    elements.append(Spacer(1, SECTION_BUFFER_HEIGHT))
                    elements.append(Paragraph(field["label"], styles["BoldLeft"]))
            ...
            for index, group in enumerate(field["children"]):
                ...
                for child_field in group:
                    if child_field["type"] in ["radiobuttons", "select"]:
                        row.append(
                            Paragraph(
                                dict([i.values() for i in child_field["options"]]).get(
                                    child_field["data"], "Not Specified"
                                ),
                                styles["Left"],
                            )
                        )
...
def _create_letter_header_footer(canvas, doc):
    ...
    canvas.drawRightString(
        current_x,
        current_y + SMALL_FONTSIZE,
        f"Phone: {DPAW_PHONE} Fax: {DPAW_FAX} Email: {DPAW_EMAIL}",
    )
    ...
    canvas.drawRightString(
        current_x, current_y + SMALL_FONTSIZE * 3, "Wildlife Licensing Section"
    )


def _format_name(user, include_first_name=False):
    if user.title:
        return f"{user.title} {user.get_full_name() if include_first_name else user.last_name}"
    else:
...
def _create_letter_address(licence):
    ...
    address_elements.append(
        Paragraph(
            _format_name(addressee, include_first_name=True), styles["LetterLeft"]
        )
    )
...
def _create_cover_letter(cover_letter_buffer, licence, site_url):
    ...
    doc = BaseDocTemplate(
        cover_letter_buffer, pageTemplates=[every_cover_letter_template], pagesize=A4
    )
    ...
    elements += _create_letter_paragraph(
        f"{licence.licence_type.name}", style="LetterBoldLeft"
    )
...
def _create_licence_renewal_elements(licence):
    ...
    licence_renewal_elements += _create_letter_paragraph(
        f"Dear {_format_name(licence.holder)}"
    )
    ...
    licence_renewal_elements += _create_letter_paragraph(
        f"{licence.licence_type.name} <{licence.reference}>",
    ...
    licence_renewal_elements += _create_letter_paragraph(
        "Please note that if you have outstanding returns, these "
        "are required to be submitted before the licence can be "
        "renewed."
    )

    licence_renewal_elements += _create_letter_paragraph(
        "If you have any queries, please contact the Wildlife "
...
def _create_licence_renewal(licence_renewal_buffer, licence, site_url):
    licence_renewal_frame = Frame(
        LETTER_PAGE_MARGIN,
        LETTER_PAGE_MARGIN,
        PAGE_WIDTH - 2 * LETTER_PAGE_MARGIN,
        PAGE_HEIGHT - 160,
        id="LicenceRenewalFrame",
    )
    licence_renewal_template = PageTemplate(
        id="LicenceRenewalFrame",
        frames=[licence_renewal_frame],
        onPage=_create_letter_header_footer,
    )

    doc = BaseDocTemplate(
        licence_renewal_buffer, pageTemplates=[licence_renewal_template], pagesize=A4
    )

    elements = (
        _create_letter_address(licence)
        + [Spacer(1, LETTER_ADDRESS_BUFFER_HEIGHT)]
        + _create_licence_renewal_elements(licence)
        + _create_letter_signature()
    )
...
def _create_bulk_licence_renewal(licences, site_url, buf=None):
    bulk_licence_renewal_frame = Frame(
        LETTER_PAGE_MARGIN,
        LETTER_PAGE_MARGIN,
        PAGE_WIDTH - 2 * LETTER_PAGE_MARGIN,
        PAGE_HEIGHT - 160,
    ...
    if buf is None:
        buf = BytesIO()
    doc = BaseDocTemplate(
        buf, pageTemplates=[bulk_licence_renewal_template], pagesize=A4
    )
...
def create_licence_pdf_document(
    filename, licence, application, site_url, original_issue_date
):
    licence_buffer = BytesIO()

    _create_licence(licence_buffer, licence, application, site_url, original_issue_date)

    document = LocalDocument.objects.create(name=filename)
