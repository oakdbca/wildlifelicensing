define([
  ...
  function initApplicationsTable() {
    var applicationsTableOptions = $.extend(
        { order: [[0, "desc"]] },
        tableOptions,
        {
          ajax: {
            ...
            data: function (d) {
              ...
              d.filters = $(
                options.selectors.applicationsFilterForm
              ).serializeArray();
            },
            ...
            error: function (xhr, textStatus, thrownError) {
              window.console.log(
                "Error while loading applications data:",
                thrownError,
                textStatus,
                xhr.responseText,
                xhr.status
    ...
    if (options.data.applications.tableOptions) {
      $.extend(
        ...
        options.data.applications.tableOptions
      );
    }
  ...
  function initApplicationsFilters() {
    ...
    // licence type
    if (
      $applicationsLicenceTypeFilter.length &&
      data.applications.filters.licence_type
    ) {
      _.forEach(
        data.applications.filters.licence_type.values,
        function (value) {
      ...
      if (data.applications.filters.licence_type.selected) {
        $applicationsLicenceTypeFilter.val(
          data.applications.filters.licence_type.selected
        );
      }
    ...
    if ($applicationsStatusFilter.length && data.applications.filters.status) {
      ...
      if (data.applications.filters.status.selected) {
        $applicationsStatusFilter.val(
          data.applications.filters.status.selected
        );
      }
    ...
    // assignee filter
    if (
      $applicationsAssigneeFilter.length &&
      ...
      if (data.applications.filters.assignee.selected) {
        $applicationsAssigneeFilter.val(
          data.applications.filters.assignee.selected
        );
      }
  ...
  function initLicencesTable() {
    var licencesTableOptions = $.extend(
        { order: [[0, "desc"]] },
        tableOptions,
        {
          ajax: {
            ...
            data: function (d) {
              ...
              if ($(options.selectors.licencesFilterForm)) {
                d.filters = $(
                  options.selectors.licencesFilterForm
                ).serializeArray();
              }
            },
            dataSrc: function (data) {
            ...
            error: function (xhr, textStatus, thrownError) {
              window.console.log(
                "Error while loading licences data:",
                thrownError,
                textStatus,
                xhr.responseText,
                xhr.status
  ...
  function initLicencesFilters() {
    ...
    if (
      $licencesLicenceTypeFilter &&
      $licencesLicenceTypeFilter.length &&
      data.licences.filters.licence_type
    ) {
      ...
      if (data.licences.filters.licence_type.selected) {
        $licencesLicenceTypeFilter.val(
          data.licences.filters.licence_type.selected
        );
      }
    ...
    if (
      $licencesStatusFilter &&
      $licencesStatusFilter.length &&
      data.licences.filters.status
    ) {
        ...
        ajax: {
          ...
          data: function (d) {
            ...
            if ($(options.selectors.returnsFilterForm)) {
              d.filters = $(
                options.selectors.returnsFilterForm
              ).serializeArray();
            }
          },
          error: function (xhr, textStatus, thrownError) {
            //Stop the data table 'Processing'.
            window.console.log(
              "Error while loading returns data:",
              thrownError,
              textStatus,
              xhr.responseText,
              xhr.status
  ...
  function initReturnsFilters() {
    ...
    if (
      $returnsLicenceTypeFilter &&
      $returnsLicenceTypeFilter.length &&
      data.returns.filters.licence_type
    ) {
      ...
      if (data.returns.filters.licence_type.selected) {
        $returnsLicenceTypeFilter.val(
          data.returns.filters.licence_type.selected
        );
      }
    ...
    if (
      $returnsStatusFilter &&
      $returnsStatusFilter.length &&
      data.returns.filters.status
    ) {
  ...
  // Helper to show/hide/toggle collapsible regions using Bootstrap 5 API when available
  // Falls back to jQuery .collapse(...) when Bootstrap isn't present yet.
  function bsCollapseShow(selector) {
    ...
    } else if ($(selector).collapse) {
      $(selector).collapse("show");
    }
  }

  function bsCollapseHide(selector) {
    var el = document.querySelector(selector);
    if (!el) {
      return;
    }
  ...
  function showTable(table) {
    if (table === "applications") {
      bsCollapseShow(options.selectors.applicationsAccordion);
      bsCollapseHide(options.selectors.licencesAccordion);
      bsCollapseHide(options.selectors.returnsAccordion);
    } else if (table === "licences") {
      bsCollapseHide(options.selectors.applicationsAccordion);
      bsCollapseShow(options.selectors.licencesAccordion);
      bsCollapseHide(options.selectors.returnsAccordion);
    } else if (table === "returns") {
      bsCollapseHide(options.selectors.applicationsAccordion);
      bsCollapseHide(options.selectors.licencesAccordion);
      bsCollapseShow(options.selectors.returnsAccordion);
    } else if (table === "none") {
      bsCollapseHide(options.selectors.applicationsAccordion);
      bsCollapseHide(options.selectors.licencesAccordion);
      bsCollapseHide(options.selectors.returnsAccordion);
  ...
  return function (moduleOptions) {
    ...
    $(function () {
      $applicationsLicenceTypeFilter = $(
        options.selectors.applicationsLicenceTypeFilter
      );
      $applicationsStatusFilter = $(options.selectors.applicationsStatusFilter);
      $applicationsAssigneeFilter = $(
        options.selectors.applicationsAssigneeFilter
      );
      $applicationsResetFilterButton = $(
        options.selectors.applicationsResetFilterButton
      );

      $licencesLicenceTypeFilter = $(
        options.selectors.licencesLicenceTypeFilter
      );
      $licencesStatusFilter = $(options.selectors.licencesStatusFilter);
      $licencesExpireAfterFilter = $(
        options.selectors.licencesExpireAfterFilter
      );
      $licencesExpireBeforeFilter = $(
        options.selectors.licencesExpireBeforeFilter
      );
      $licencesResetFilterButton = $(
        options.selectors.licencesResetFilterButton
      );
