...
class TableAssessorView(AssessorRequiredMixin, base.TablesBaseView):
    ...
    def applications_filters(self):
        status_filter_values = [("all", "All")] + [
            (v, l) for v, l in Assessment.STATUS_CHOICES
        ]
        return {
...
class DataTableApplicationAssessorView(
    OfficerOrAssessorRequiredMixin, base.DataTableBaseView
):
    """
    ...
    order_columns = [
        ...
        [
            "assigned_assessor.first_name",
            "assigned_assessor.last_name",
    ...
    columns_helpers = dict(
        **{
            ...
            "application.lodgement_number": {
                "search": lambda self, search: DataTableApplicationAssessorView._search_lodgement_number(
                    search
                ),
                "render": lambda self, instance: base.render_lodgement_number(
            ...
            "application.application_type": {
                "render": lambda self, instance: self.APPLICATION_TYPES[
                    instance.application.application_type
                ]
            },
            "application.assigned_officer": {
                "render": lambda self, instance: render_user_name(
                    instance.application.assigned_officer
                ),
                "search": lambda self, search: base.build_field_query(
                    [
            ...
            "assigned_assessor": {
                "render": lambda self, instance: render_user_name(
                    instance.assigned_assessor
                ),
                "search": lambda self, search: base.build_field_query(
                    [
                        "assigned_assessor__last_name",
                        "assigned_assessor__first_name",
                        "assigned_assessor__email",
                    ],
                    search,
                ),
            },
            "application.lodgement_date": {
                "render": lambda self, instance: base.render_date(
                    instance.application.lodgement_date
                ),
            },
            "application_pdf": {
            ...
            "action": {
                "render": lambda self, instance: DataTableApplicationAssessorView.render_action_column(
                    instance
                ),
            },
        }
    ...
    def _search_lodgement_number(search):
        # testing to see if search term contains no spaces and two
        # hyphens, meaning it's a lodgement number with a sequence
        if search and search.count(" ") == 0 and search.count("-") == 2:
            ...
            return Q(application__lodgement_number__icontains=lodgement_number) & Q(
                application__lodgement_sequence__icontains=lodgement_sequence
            )
        else:
    ...
    def filter_licence_type(value):
        return (
            Q(application__licence_type__pk=value) if value.lower() != "all" else None
        )
