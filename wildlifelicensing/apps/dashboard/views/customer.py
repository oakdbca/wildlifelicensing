...
def _get_user_applications(user):
    return Application.objects.filter(applicant_profile__user=user).exclude(
        customer_status__in=["temp", "discarded"]
    )
...
class TableCustomerView(LoginRequiredMixin, base.TablesBaseView):
    ...
    def applications_table_options(self):
        return {"pageLength": 25, "order": [[0, "desc"], [4, "desc"]]}
    ...
    def licences_table_options(self):
        return {"pageLength": 10, "order": [[4, "desc"]]}
    ...
    def returns_table_options(self):
        return {"pageLength": 10, "order": [[3, "desc"]]}
...
class DataTableApplicationCustomerView(base.DataTableApplicationBaseView):
    ...
    columns_helpers = dict(
        ...
        **{
            "lodgement_number": {
                "search": lambda self, search: DataTableApplicationCustomerView._search_lodgement_number(
                    search
                ),
                "render": lambda self, instance: base.render_lodgement_number(instance),
            },
            "action": {
                "render": lambda self, instance: DataTableApplicationCustomerView.render_action_column(
                    instance
                ),
            },
            "lodgement_date": {
...
class DataTableLicencesCustomerView(base.DataTableBaseView):
    ...
    columns_helpers = dict(
        ...
        **{
            "licence_number": {
                "search": lambda self, search: DataTableLicencesCustomerView._search_licence_number(
                    search
                ),
    ...
    def _render_status(instance):
        try:
            ...
            if (
                replacing_application.licence is not None
                and replacing_application.licence.is_issued
            ):
    ...
    def _render_action(instance):
        try:
            ...
            if (
                replacing_application.licence is None
                or not replacing_application.licence.is_issued
            ):
                if replacing_application.application_type == "amendment":
    ...
    def get_initial_queryset(self):
        ...
        return WildlifeLicence.objects.filter(holder=self.request.user).filter(
...
class DataTableReturnsCustomerView(base.DataTableBaseView):
    ...
    columns_helpers = {
        ...
        "licence": {
            ...
            "search": lambda self, search: DataTableReturnsCustomerView._search_licence_number(
                search
            ),
        },
        "action": {"render": lambda self, instance: self._render_action(instance)},
    ...
    def get_initial_queryset(self):
        return Return.objects.filter(licence__holder=self.request.user).exclude(
            status="future"
        )
