...
from wildlifelicensing.apps.returns.utils import is_return_due_soon, is_return_overdue
...
class DashboardOfficerTreeView(OfficerRequiredMixin, base.DashboardTreeViewBase):
    ...
    def _build_tree_nodes(self):
        ...
        on_behalf_pending_applications_count = (
            DataTableApplicationsOfficerOnBehalfView._get_proxy_applications(
                self.request.user
            )
            .filter(
        ...
        on_behalf_overdue_returns_count = (
            DataTableReturnsOfficerOnBehalfView._get_proxy_returns(self.request.user)
            .filter(
                DataTableReturnsOfficerOnBehalfView.filter_status(
                    TablesReturnsOfficerView.OVERDUE_FILTER
                )
            )
            .count()
        )
        ...
        total_on_behalf = (
            on_behalf_pending_applications_count + on_behalf_overdue_returns_count
        )
        if total_on_behalf > 0:
            ...
            on_behalf_node = self._create_node(
                "My proxy page", href=url, count=total_on_behalf
            )
            ...
            on_behalf_returns_node = self._create_node(
                "Overdue Returns", href=href, count=on_behalf_overdue_returns_count
            )
...
class TablesApplicationsOfficerView(OfficerRequiredMixin, base.TablesBaseView):
    ...
    def applications_columns(self):
        return [
            ...
            {"title": "Application PDF", "searchable": False, "orderable": False},
            {"title": "Action", "searchable": False, "orderable": False},
        ]

    @property
    def applications_table_options(self):
        return {"pageLength": 25, "order": [[4, "desc"], [0, "desc"]]}
    ...
    def applications_filters(self):
        status_filter_values = (
            [("all", "All")]
            + [(self.STATUS_PENDING, self.STATUS_PENDING.capitalize())]
...
class DataTableApplicationsOfficerView(
    OfficerRequiredMixin, base.DataTableApplicationBaseView
):
    columns = [
        "lodgement_number",
        "licence_type",
    ...
    order_columns = [
        ...
        "lodgement_date",
        [
            "assigned_officer.first_name",
            "assigned_officer.last_name",
    ...
    columns_helpers = dict(
        base.DataTableApplicationBaseView.columns_helpers.items(),
        **{
            "lodgement_number": {
                "search": lambda self, search: DataTableApplicationsOfficerView._search_lodgement_number(
                    search
                ),
                "render": lambda self, instance: base.render_lodgement_number(instance),
            },
            "lodgement_date": {
                "render": lambda self, instance: base.render_date(
                    instance.lodgement_date
                )
            },
            "assigned_officer": {
                ...
                "render": lambda self, instance: base.render_user_name(
                    instance.assigned_officer
                ),
            },
            "payment": {
                "render": lambda self, instance: base.render_payment(
                    instance,
            ...
            "application_pdf": {
                "render": lambda self, instance: base.render_application_document(
                    instance
                )
            },
            "action": {
                "render": lambda self, instance: DataTableApplicationsOfficerView._render_action_column(
                    instance
                ),
            },
        },
    ...
    def filter_status(value):
        ...
        if value.lower() == TablesApplicationsOfficerView.STATUS_PENDING:
            return Q(
                processing_status__in=DataTableApplicationsOfficerView._get_pending_processing_statuses()
            )
        return (
            Q(processing_status=value)
            if value != "all"
            else ~Q(customer_status="draft")
        )
    ...
    def _search_lodgement_number(search):
        ...
        if search and search.count(" ") == 0 and search.count("-") == 2:
            ...
            lodgement_number, lodgement_sequence = "-".join(components[:2]), "-".join(
                components[2:]
            )

            return Q(lodgement_number__icontains=lodgement_number) & Q(
                lodgement_sequence__icontains=lodgement_sequence
            )
        else:
    ...
    def _render_action_column(obj):
        issued = (
            obj.processing_status == "issued"
            and obj.licence is not None
            and obj.licence.licence_document is not None
        )
    ...
    def get_initial_queryset(self):
        return Application.objects.exclude(
            processing_status__in=["draft", "temp"]
        ).exclude(customer_status="temp")
...
class TablesOfficerOnBehalfView(OfficerRequiredMixin, base.TablesBaseView):
    ...
    applications_data_url_lazy = reverse_lazy(
        "wl_dashboard:data_application_officer_onbehalf"
    )
    ...
    def returns_table_options(self):
        return {"pageLength": 25, "order": [[0, "asc"]]}
    ...
    def returns_filters(self):
        status_filter_values = [
            (
                TablesReturnsOfficerView.STATUS_FILTER_ALL_BUT_DRAFT_OR_FUTURE,
                "All (but draft or future)",
            ),
            (
                TablesReturnsOfficerView.OVERDUE_FILTER,
...
class DataTableApplicationsOfficerOnBehalfView(
    OfficerRequiredMixin, base.DataTableApplicationBaseView
):
    columns = [
        "lodgement_number",
    ...
    def _get_proxy_applications(user):
        return Application.objects.filter(proxy_applicant=user).exclude(
            customer_status="temp"
        )
    ...
    def filter_status(value):
        if value.lower() == TablesApplicationsOfficerView.STATUS_PENDING:
            return Q(
                processing_status__in=DataTableApplicationsOfficerOnBehalfView._get_pending_processing_statuses()
            )
        else:
...
class TablesLicencesOfficerView(OfficerRequiredMixin, base.TablesBaseView):
    ...
    def licences_columns(self):
        return [
            ...
            {"title": "Issue Date"},
            {"title": "Expiry Date"},
            {"title": "Licence PDF", "searchable": False, "orderable": False},
    ...
    def licences_table_options(self):
        return {"pageLength": 25, "order": [[3, "desc"], [0, "desc"]]}
    ...
    def get_licences_context_data(self):
        result = super().get_licences_context_data()
        # specific stuff
...
class DataTableLicencesOfficerView(OfficerRequiredMixin, base.DataTableBaseView):
    ...
    columns_helpers = dict(
        base.DataTableBaseView.columns_helpers.items(),
        **{
            "licence_number": {
                "search": lambda self, search: DataTableLicencesOfficerView._search_licence_number(
                    search
                ),
    ...
    def filter_status(value):
        ...
        if value == TablesLicencesOfficerView.STATUS_FILTER_ACTIVE:
            ...
            return (
                ...
                & Q(end_date__lte=today + datetime.timedelta(days=30))
            )
    ...
    def _search_licence_number(search):
        ...
        if search and search.count(" ") == 0 and search.count("-") == 2:
            components = search.split("-")
            licence_number, licence_sequence = "-".join(components[:2]), "-".join(
                components[2:]
            )

            return Q(licence_number__icontains=licence_number) & Q(
                licence_sequence__icontains=licence_sequence
            )
        else:
    ...
    def _render_status(instance):
        ...
        if instance.end_date is not None:
            ...
            message = "The licence ref:{ref} pk:{pk} has no end date!".format(
                ref=instance.reference, pk=instance.pk
            )
    ...
    def _render_action(instance):
        ...
        if instance.end_date is not None:
            ...
            if instance.end_date >= datetime.date.today():
                return '<a href="{}">Amend</a> / <a href="{}">Reissue</a>'.format(
                    amend_url, reissue_url
                )
            else:
                return "N/A"
        else:
    ...
    columns_helpers = dict(
        ...
        **{
            "licence.licence_type": {
                ...
                "search": lambda self, search: base.build_field_query(
                    [
                        "licence__licence_type__short_name",
            ...
            "licence.profile.user": {
                "render": lambda self, instance: base.render_user_name(
                    instance.licence.profile.user, first_name_first=False
                ),
                "search": lambda self, search: base.build_field_query(
                    [
                        "licence__profile__user__last_name",
                        "licence__profile__user__first_name",
                    ],
                    search,
                ),
            },
            ...
            "status": {"render": lambda self, instance: self._render_status(instance)},
            "licence_number": {
                "render": lambda self, instance: base.render_licence_number(
                    instance.licence
                ),
                "search": lambda self, search: DataTableReturnsOfficerView._search_licence_number(
                    search
                ),
            },
            "action": {"render": lambda self, instance: self._render_action(instance)},
...
class DataTableReturnsOfficerOnBehalfView(DataTableReturnsOfficerView):
    ...
    def _get_proxy_returns(user):
        return Return.objects.filter(
            licence__in=WildlifeLicence.objects.filter(
                application__proxy_applicant=user
            )
        )
