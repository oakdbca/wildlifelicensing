...
from wildlifelicensing.apps.main.models import BaseLicenceType
...
class TablesBaseView(TemplateView):
    ...
    def get_licence_types_values():
        return [("all", "All")] + [
            (lt.pk, lt.display_name) for lt in BaseLicenceType.objects.all()
        ]
...
class DataTableBaseView(LoginRequiredMixin, BaseDatatableView):
    ...
    def _get_order_config_array(self):
        """
        From the order parameters passed in the request, return a
        datatable configuration array that can be stored
        in session. See https://datatables.net/reference/option/order
        Example:
        Received params:
        (u'order[0][column]', u'4'), (u'order[0][dir]', u'desc'),
        (u'order[1][column]', u'0'), (u'order[1][dir]', u'desc')
        Returned array:
        [[4, 'desc'], [0, 'desc']]
        :return:
        """
