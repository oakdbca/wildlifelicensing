...
from wildlifelicensing.apps.main.mixins import InvoiceOwnerMixin
from wildlifelicensing.apps.main.mock_apis import get_invoice, get_product
...
from wildlifelicensing.apps.payments.utils import generate_product_title
    ...
    def get(self, request):
        ...
        if form.is_valid():
            ...
            if response.status_code == 200:
                ...
                messages.error(
                    request,
                    "There was an error while generating the payment report:<br>{}".format(
                        response.content
                    ),
                )
...
class InvoicePDFView(InvoiceOwnerMixin, generic.View):
    ...
    def get_object(self):
        invoice = get_invoice(self.kwargs["reference"])
        return invoice
