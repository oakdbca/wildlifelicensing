...
class PaymentsReportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        delta = (
            ...
            if today_ten_pm_aest_local.weekday() != FR.weekday
            else relativedelta(weekday=FR(-2))
        )
        end = today_ten_pm_aest_local + delta
        start = end + relativedelta(weeks=-1)

        banked_start = (start - datetime.timedelta(days=start.weekday())).replace(
            hour=0, minute=0
        )
        banked_end = (banked_start + datetime.timedelta(days=6)).replace(
