...
class Remittance(Flowable):
    ...
    def __payment_line(self):
        ...
        canvas.drawString(
            cheque_x,
            cheque_y,
            "Make cheque payable to: Department of Biodiversity, Conservation and Attractions",
        )
        cheque_y -= 15
        canvas.drawString(
            cheque_x,
            cheque_y,
            "Mail to: Department of Biodiversity, Conservation and Attractions",
        )
        cheque_y -= 15
        ...
        if settings.BPAY_ALLOWED:
            ...
            canvas.drawString(
                biller_x + 5, info_y, f"Biller Code: {self.invoice.biller_code}"
            )
            canvas.drawString(
                biller_x + 5, info_y - 20, f"Ref: {self.invoice.reference}"
            )
            # Bpay Info string
    ...
    def __footer_line(self):
        ...
        current_y -= 2 * inch
        total_gst_tax = (
            self.invoice.order.total_incl_tax - self.invoice.order.total_excl_tax
        )
        ...
        # - calculate_excl_gst(self.invoice.amount)))
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    # canvas.drawString(current_x + invoice_details_offset, current_y
    # - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 5, currency(invoice.amount
    # - calculate_excl_gst(invoice.amount)))
    canvas.drawString(
        current_x + invoice_details_offset,
        current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 5,
        currency(total_gst_tax),
    )
    canvas.drawRightString(
        current_x + 20,
...
def _create_invoice(invoice_buffer, invoice):
    ...
    for discount in discounts:
        data.append(["", discount.offer, "", "", f"-${discount.amount}"])
        val += 1
    t = Table(
        data,
