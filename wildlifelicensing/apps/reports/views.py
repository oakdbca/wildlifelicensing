...
class ReportHelper:
    ...
    def export_licence_type(licence_type):
        ...
        return (licence_type.display_name, licence_type.code)
...
class ApplicationsReportView(OfficerRequiredMixin, View):
    ...
    def export(application):
        return (
            ...
            (
                render_user_name(application.proxy_applicant)
                if application.proxy_applicant
                else ""
            ),
            (
                render_user_name(application.assigned_officer)
                if application.proxy_applicant
                else ""
            ),
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            qs = Application.objects.filter(
                lodgement_date__range=(from_date, to_date)
            ).exclude(processing_status="draft")
            wb = ReportHelper.to_workbook(
                "Applications", self.ALL_HEADERS, self.row_generator(qs)
            )
            filename = f"applications_{from_date}-{to_date}.xlsx"
...
class LicencesReportView(OfficerRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            wb = ReportHelper.to_workbook(
                "Licences", self.ALL_HEADERS, self.row_generator(qs)
            )
            filename = f"licences_{from_date}-{to_date}.xlsx"
            return excel.WorkbookResponse(wb, filename)
        else:
            messages.error(request, form.errors)
            redirect("wl_reports:reports")
...
class ReturnsReportView(OfficerRequiredMixin, View):
    ...
    ALL_HEADERS = ReportHelper.LICENCE_TYPE_HEADERS + RETURNS_HEADERS

    def row_generator(self, returns):
        for ret in returns:
            row = ReportHelper.export_licence_type(
                ret.licence.licence_type
            ) + self.export(ret)
            yield row
    ...
    def get(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            wb = ReportHelper.to_workbook(
                "Returns", self.ALL_HEADERS, self.row_generator(qs)
            )
            filename = f"returns_{from_date}-{to_date}.xlsx"
            return excel.WorkbookResponse(wb, filename)
        else:
            messages.error(request, form.errors)
            redirect("wl_reports:reports")
