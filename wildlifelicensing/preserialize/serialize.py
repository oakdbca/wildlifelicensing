...
from .utils import convert_to_camel, get_field_value, parse_selectors
...
def _defaults(options):
    if "key_map" in options and "aliases" not in options:
        warnings.warn(
            'The "key_map" option has been renamed to "aliases"', DeprecationWarning
        )
        options["aliases"] = options.pop("key_map")

    if "key_prefix" in options and "prefix" not in options:
        warnings.warn(
            'The "key_prefix" option has been renamed to "prefix"', DeprecationWarning
        )
...
def model_to_dict(instance, **options):
    ...
    for alias in options["fields"]:
        ...
        value = get_field_value(
            instance, accessor, allow_missing=options["allow_missing"]
        )
